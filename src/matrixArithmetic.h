/*
    ...
    Matrix Arithmetic Functions
        All functions pertain to arithmetic on matrices
...
bool matrix_compare(const matrix a, const matrix b)
{
    ...
    for(int i = 0; i < a.n; i++)
    {
        for(int j = 0; j < a.n; j++)
        {
            if(fabs((a.arr[i][j]) - (b.arr[i][j])) > DECIMAL_PRECISION) // Check two paired elements for equality
            {
                return false; // If two elements do not compare, return false
            }
        }
    }
    return true; // All elements are the same, equal matrices
}
...
matrix matrix_multiply(const matrix a, const matrix b)
{
    ...
    matrix_io_printToConsole(a);
    matrix_io_printToConsole(b);
